module.exports = {
  ...
  execute(message, args, fs) {
    ...
    const file_dec = fs.openSync("./json/scribbleNames.json");
    const words = JSON.parse(fs.readFileSync("./json/scribbleNames.json"));
...
function writeToFile(words, fs) {
  ...
  fs.writeFile("./json/scribbleNames.json", newData, error => {
    if (error) {
